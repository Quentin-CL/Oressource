...
function filieres_sorties(PDO $bdd): array
{
  ...
  return array_map(function ($filiere) {
    $a = array_filter(explode('a', $filiere['id_type_dechet_evac']), function ($e): bool {
      return $e !== '';
    });
    $b = array_map(function (int $e): int {
      return $e;
    }, $a);
...
function filieres_sorties_id(PDO $bdd, int $id): array
{
  ...
  $r['accepte_type_dechet'] = array_flip(array_filter(
    explode('a', $r['id_type_dechet_evac']),
    function ($e) {
      return $e !== '';
    }
  ));
 ...
 */
function chiffre_affaire_mode_paiement(
  PDO $bdd,
  string $start,
  string $stop,
...
function viz_caisse(PDO $bdd, int $id_point_vente, int $offset): array
{
  $sql = "SELECT
    ...
    SUM(pv.quantite * pv.masse) as masse
  from ventes
  inner join vendus
    on vendus.id_vente = ventes.id
...
function viz_collectes(PDO $bdd, int $id_point_collecte, int $offset): array
{
  ...
  $reqCollectes->bindValue('id_point_collecte', $id_point_collecte, PDO::PARAM_INT);
  $reqCollectes->bindValue('offset', $offset, PDO::PARAM_INT);
  $reqCollectes->execute();
  $resultat = $reqCollectes->fetchAll(PDO::FETCH_ASSOC);
  $reqCollectes->closeCursor();
  return $resultat;
}
