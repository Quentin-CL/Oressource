    ...
    <div class="col-md-4 col-md-offset-1">
      <div class="panel panel-info">
        ...
        <div class="panel-body">
          ...
          ?>
            <div class='btn-group'>
              <button type="button" class="btn btn-default btn-lg <?= (!empty($objs)) ? 'dropdown-toggle' : '' ?>" <?php if (!empty($objs)) { ?> data-toggle="dropdown" <?php } else { ?> onclick='update_state({ type: <?= json_encode($d, JSON_NUMERIC_CHECK) ?> });
                            return false;' <?php } ?> style="margin-left:8px; margin-top:16px; background-color:<?= $d['couleur'] ?>; color: white; font-weight: bold">
                <?= $d['nom'] ?>
              </button>
              <?php if (!empty($objs)) { ?>
          ...
          <div class="collapse" id="collapserendu">
            <ul class="list-group list-group-item-warning">
              <li class="list-group-item">
                Somme due:
                <input type="text" class="form-control" placeholder="€" name="somme" id="somme" disabled>
              </li>
              <li class="list-group-item list-group-item-success">
                <b>Réglement</b>
                <input type="text" class="form-control" placeholder="€" name="reglement" id="reglement" oninput="update_rendu()">
              </li>
              <li class="list-group-item list-group-item-danger">
                <b>A rendre</b>
                <input type="text" class="form-control" placeholder="€" name="difference" id="difference" disabled>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div class="panel-body">
          ...
          <div id="moyens" class="btn-group" data-toggle="buttons">
            <?php foreach ($moyens_paiement as $moyen) { ?>
              <label class="btn ors_btn_pay btn-default <?= $moyen['id'] === 1 ? 'active' : '' ?>" onclick="moyens(<?= $moyen['id'] ?>);" style="background-color: <?= $moyen['couleur']; ?>">
                <input type='radio' name='paiement' id='paiement' <?= $moyen['id'] === 1 ? 'checked' : '' ?>>
                <?= $moyen['nom'] ?></label>
            <?php } ?>
          </div>
        </div>
