...
?>
          ...
          <div class="panel panel-danger">
            ...
            <div class="panel-body">
              <?php foreach (filter_visibles(types_dechets($bdd)) as $donnees) { ?>
                <div class="btn-group">
                  <button type="button" class="btn btn-lg btn-default dropdown-toggle" data-toggle="dropdown" style="margin-left:8px; margin-top:16px; background-color:<?= $donnees['couleur']; ?>; color: white; font-weight: bold">
                    <?= $donnees['nom']; ?>
                  </button>
            ...
            <div class="panel-body">
              <form action="../moteur/remboursement_post.php" id="formulaire" method="post">
                <?php if (is_allowed_saisie_date() && (strpos($_SESSION['niveau'], 'e') !== false)) { ?>
                  ...
                  <input type="date" id="antidate" name="antidate" value="<?= date('Y-m-d'); ?>">
                  <br><br>
                <?php } ?>
  ...
  <script type="text/javascript">
    ...
    function often(that) {
      if (isNaN(parseInt(document.getElementById('id_type_objet').value))) {} else {
    ...
    function ajout() {
      if (isNaN((parseFloat(document.getElementById('prix').value) * parseFloat(document.getElementById('quantite').value)).toFixed(2))) {} else {
